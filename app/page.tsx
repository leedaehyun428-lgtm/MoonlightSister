...
export default function Home() {
  ...
  const clearChat = () => {
    if (confirm('대화를 삭제하고 처음부터 다시 할까?')) {
      setMessages([]);
    ...
    <div className="flex flex-col h-screen bg-gradient-to-b from-slate-900 via-purple-900 to-slate-900 text-white font-sans overflow-hidden">
      ...
      <header className="fixed top-0 w-full z-10 flex justify-between items-center p-4 bg-white/5 backdrop-blur-md border-b border-white/10 shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <h1 className="text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-200 to-pink-200">달빛 언니</h1>
          </div>
        </div>
        <div className="flex gap-2">
           <button onClick={clearChat} className="text-xs text-gray-400 hover:text-white px-3 py-1 border border-white/20 rounded-full">비우기</button>
           <button onClick={handleShare} className="text-xs bg-indigo-500 hover:bg-indigo-600 text-white px-3 py-1 rounded-full shadow-lg flex items-center gap-1"><span>🔗 링크</span></button>
           <button onClick={handleKakaoShare} className="bg-[#FEE500] text-[#191919] text-xs font-bold px-3 py-1 rounded-full shadow-lg flex items-center gap-1 hover:bg-[#Fdd835]"><span>💬 카톡</span></button>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto pt-20 pb-24 px-4 space-y-6 scrollbar-hide">
          ...
          <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
            ...
            <div className={`relative max-w-[90%] rounded-2xl p-4 shadow-md backdrop-blur-sm overflow-hidden ${
              ...
              {msg.isLocked && (
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-purple-950/90 backdrop-blur-md p-4 text-center">
                  ...
                  {msg.coupangLink ? (
                    ...
                    <div className="text-xs text-gray-400 animate-pulse">🎁 선물 포장 중... (잠시만!)</div>
                  )}
              ...
              <div className={msg.isLocked ? 'blur-sm opacity-50' : ''}>
                  ...
                  {msg.cardKeywords && (
                    <div className="mt-4 bg-black/20 rounded-xl p-4 border border-white/10 text-sm space-y-4">
                      ...
                      <div className="space-y-3 text-xs leading-relaxed text-gray-300">
                        <div className="bg-white/5 p-2 rounded-lg">
                          <strong className="text-blue-300 block mb-1">📖 카드 정보</strong>
                          {msg.cardDescription}
                        </div>
                        <div>
                          <strong className="text-purple-300 block mb-1">🧐 상황 분석</strong>
                          {msg.cardAnalysis}
                        </div>
                        <div>
                          <strong className="text-yellow-300 block mb-1">💡 언니의 조언</strong>
                          {msg.cardAdvice}
                        </div>
                      </div>
                    </div>
        ...
        {isLimitReached && (
          <div className="text-center py-4 animate-fade-in">
            <p className="text-sm text-gray-400 mb-2">오늘 상담 횟수를 모두 썼어. (7회)</p>
            <button onClick={clearChat} className="bg-gray-700 hover:bg-gray-600 text-white text-xs px-4 py-2 rounded-full transition">🔄 새로운 상담 시작하기</button>
          </div>
        )}
      ...
      <footer className="fixed bottom-0 w-full bg-slate-900/80 backdrop-blur-lg border-t border-white/5 p-4 pb-6">
        <div className="max-w-3xl mx-auto flex items-center bg-gray-800/50 rounded-full px-2 border border-white/10 focus-within:border-purple-500/50 transition-colors">
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && sendMessage()} placeholder={isLimitReached ? "상담이 종료되었습니다." : "마음껏 털어놔 봐..."} disabled={loading || isLimitReached} className="flex-1 bg-transparent p-3 text-white placeholder-gray-500 focus:outline-none text-sm disabled:text-gray-600" />
          <button onClick={sendMessage} disabled={loading || isLimitReached} className={`bg-purple-600 hover:bg-purple-500 text-white rounded-full w-10 h-10 flex items-center justify-center transition shadow-lg ${loading || isLimitReached ? 'opacity-50 cursor-not-allowed' : ''}`}><span className="text-lg">➤</span></button>
        </div>
      </footer>
    </div>
  );
